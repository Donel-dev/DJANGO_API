...
def product_api_view(request, pk=None, *args, **kwargs):
    ...
    if request.method == 'GET':
        if pk is not None:
            ...
            serializer = ProductSerializer1(product, context=context) #On peut aussi utiliser ProductSerializer2(product) mais dans ce cas ci on doit definir les champs que l'on veut serialiser et deserialiser dans le serializer, alors que dans ProductSerializer1 on utilise un model serializer qui va automatiquement definir les champs à partir du model, et on peut aussi ajouter des champs qui ne sont pas dans le model, ou faire des calculs à partir des champs du model pour definir des champs qui ne sont pas dans le model, et ces champs seront pris en compte lors de la serialisation et de la deserialisation, context sert à passer le contexte de la requete au serializer pour que le serializer puisse construire les liens vers les detail view de l'instance du model, on fait cela en passant le contexte de la requete au serializer, et en utilisant HyperlinkedIdentityField dans le serializer pour construire les liens vers les detail view de l'instance du model
            return Response(serializer.data, status=status.HTT) #On peut aussi utiliser return Response(serializer.data) mais c'est mieux de préciser le status de la réponse pour que le client puisse savoir si la requete a réussi ou pas, et quel type de reponse il doit attendre serialiser.data est une representation en format json de l'instance du model, et c'est ce que le client va recevoir dans la reponse de la requete GET
    ...
    if request.method == 'POST':
        ...
        context = {'request': request} #On peut aussi passer le contexte de la requete au serializer pour que le serializer puisse construire les liens vers les detail view de l'instance du model, on fait cela en passant le contexte de la requete au serializer, et en utilisant HyperlinkedIdentityField dans le serializer pour construire les liens vers les detail view de l'instance du model
        serializer = ProductSerializer1(data=request.data, context=context) #request.data est un dictionnaire qui contient les données envoyées par le client dans la requete POST, et on passe ce dictionnaire au serializer pour qu'il puisse faire la deserialisation et la validation des données, si les données sont valides le serializer va creer une instance du model à partir des données validées, sinon il va renvoyer les erreurs de validation
